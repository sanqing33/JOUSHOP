<template>
  <!-- 搜索框 -->
  <view style="width: 90vw; margin: 5px auto">
    <up-search
      placeholder="日照香炉生紫烟"
      v-model="keyword"
      height="40"
  ...
  >
    <template #right>
      ...
      >
        筛选
        <up-icon name="grid" color="#606266" size="21" bold></up-icon>
      </view>
    </template>
  </up-tabs>
...
import type { SearchGoodsData } from "@/types/search";

import { onLoad } from "@dcloudio/uni-app";
import { reactive, ref } from "vue";

const list1 = reactive([{ name: "推荐" }, { name: "销量" }, { name: "价格" }]);
...
const goods1 = reactive([
  {
    ...
    shop: "李白杜甫白居易旗舰店",
    image:
      "https://b0.bdstatic.com/2bb0d5086b1010e82cf1b1edbb15820e.jpg@h_1280",
  },
  {
    price: 75,
    title: "望长城内外，惟余莽莽",
...
const searchData = reactive<SearchGoodsData>({
  page: 1,
  pageSize: 10,
...
const getSearchGoods = async () => {
  const res = await searchGoodsAPI(searchData);
  console.log(res);
};
